...
func main() {
	var groupCounts = make(map[int]Counts)
	for i, group := range strings.Split(input, "\n\n") {
		var uniqueRunes = make(map[rune]int)
		var groupSize = 1
		for _, r := range group {
			if r == '\n' {
				groupSize++
		...
		fmt.Println(i, len(uniqueRunes))
		var allChose = 0
		for _, count := range uniqueRunes {
			if count == groupSize {
				allChose++
			}
		}
		groupCounts[i] = Counts{len(uniqueRunes), allChose}
	}

	var uniqueSum = 0
	var allSum = 0

	for _, counts := range groupCounts {
		uniqueSum += counts.unique
		allSum += counts.all
	}
	fmt.Println(uniqueSum, allSum)
}
