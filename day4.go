...
type MinMaxIntValidator struct {
	Min int
	Max int
}

func (v MinMaxIntValidator) IsValid(s string) bool	 {
	var i, err = strconv.Atoi(s)
	if err != nil {
		return false
	}
	return i >= v.Min && i <= v.Max
}
...
func isHgtValid(hgt string) bool {
	...
	if hgt[len(hgt)-2:] == "cm" {
		return MinMaxIntValidator{150, 193}.IsValid(string(hgt[:len(hgt)-2]))
	} else {
		return MinMaxIntValidator{59, 76}.IsValid(string(hgt[:len(hgt)-2]))
	}
}
...
func (rv RegexValidator) isValid(s string) bool {
	var valid = rv.r.MatchString(s)
	if !valid {
		fmt.Println(s)
	}
	return valid
...
func isPassportValid2(passport map[string]string) bool {
	var requiredFields = map[string]func(string)bool{
		"byr": MinMaxIntValidator{1920, 2002}.IsValid,
		"iyr": MinMaxIntValidator{2010, 2020}.IsValid,
		"eyr": MinMaxIntValidator{2020, 2030}.IsValid,
		"hgt": isHgtValid,
		"hcl": RegexValidator{regexp.MustCompile("^#[0-9a-f]{6}$")}.isValid,
		"ecl": RegexValidator{regexp.MustCompile("^(amb|blu|grn|gry|brn|hzl|oth)$")}.isValid,
		"pid": RegexValidator{regexp.MustCompile("^[0-9]{9}$")}.isValid,
	}
	for field, isValid := range requiredFields {
		if !isValid(passport[field]) {
			return false
		}
	}
	return true
...
func parseInput(s string) []map[string]string {
	var passportStrings = strings.Split(s, "\n\n")
	var passports = []map[string]string{}
	for _, passportString := range passportStrings {
		passports = append(passports, parsePassport(passportString))
	}
	return passports
}

func splitString(s string, sep string) (string, string) {
...
func parsePassport(s string) map[string]string {
	...
	for _, part := range parts {
		var key, value = splitString(strings.TrimSpace(part), ":")
