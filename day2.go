...
func main() {
	valid := 0
	for i := range input {
		var policy, password = parseString(input[i])
		if applyPolicy2(policy, password) {
			valid += 1
		}
	}
	fmt.Println(valid)
...
func applyPolicy(policy PasswordPolicy, password string) bool {
	occurences := strings.Count(password, policy.Character)
	return occurences >= policy.Min && occurences <= policy.Max
}

func applyPolicy2(policy PasswordPolicy, password string) bool {
	return (password[policy.Min - 1] == policy.Character[0]) != (password[policy.Max - 1] == policy.Character[0])
}
...
func parseString(s string) (PasswordPolicy, string) {
	var policyString, passwordString = splitString(s, ":")
	var policy = parsePasswordPolicy(policyString)
	var password = strings.TrimSpace(passwordString)
	return policy, password
}


func parsePasswordPolicy(s string) PasswordPolicy {
	var dashedRange, character = splitString(s, " ")
	var min, max = splitString(dashedRange, "-")
